...
func (s *Server) handleCreateCollection(w http.ResponseWriter, r *http.Request) {
	...
	c, err := s.Store.CreateCollection(r.Context(), u.ID, req.Name)
	if err != nil {
		fail(w, err)
		return
	}
...
// collectionAccess loads a collection the caller owns.
func (s *Server) collectionAccess(w http.ResponseWriter, r *http.Request) (*store.Collection, bool) {
	u := auth.UserFrom(r.Context())
	c, err := s.Store.CollectionOwnedBy(r.Context(), chi.URLParam(r, "collectionID"), u.ID)
	if err != nil {
		fail(w, err)
		return nil, false
	}
